...
import com.ijiaban.actionbar.smoothprogress.SmoothProgressBar;
import com.ijiaban.actionbar.smoothprogress.SmoothProgressBarUtils;
 ...
 *
 */
@SuppressLint("NewApi") public class DefaultHeaderTransformer extends HeaderTransformer {
	...
	private void setupViewsFromStyles(Activity activity, View headerView) {
		...
		// Retrieve the Action Bar background from the app theme or the Action
		// Bar's style (see #93)
		Drawable bg = styleAttrs
		...
		if (styleAttrs
				.hasValue(R.styleable.PullToRefreshHeader_ptrSmoothProgressBarStyle)) {
			int spbStyleRes = styleAttrs.getResourceId(
					R.styleable.PullToRefreshHeader_ptrSmoothProgressBarStyle,
					0);
			if (spbStyleRes != 0)
